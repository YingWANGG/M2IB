...
class InformationBottleneck(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        # Normalize signal to simplify the computation of the KL-divergence
        x_norm = (x - self.mean) / self.std
        masked_mu = x_norm * lamb
        masked_var = (1-lamb)**2
        self.buffer_capacity = self._calc_capacity(masked_mu, masked_var)
        t_norm = self._sample_t(masked_mu, masked_var)
        t = t_norm * self.std + self.mean
        return (t,)


class IBAInterpreter:
    def __init__(self, model, estim: Estimator, fitting_estimator, beta=10, steps=10, lr=1, batch_size=10, progbar=False):
        ...
        self.fitting_estimator = fitting_estimator
